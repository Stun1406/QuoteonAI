...
// ── City base rates (CHN Investment Group LLC Rate Sheet) ─────────────────────
export const DRAYAGE_CITY_RATES: Record<string, DrayageCityRate> = {
  ...
  'COLTON':             { base: 750,  dist: 62, requiresDropFee: true, dropFeeAmount: 300 },
  ...
  'EAST LOS ANGELES':   { base: 580,  dist: 18 },
  'EASTVALE':           { base: 700,  dist: 48 },
  'EL MONTE':           { base: 510,  dist: 22 },
  'EL SEGUNDO':         { base: 480,  dist: 14 },
  'FONTANA':            { base: 680,  dist: 52 },
  'FOUNTAIN VALLEY':    { base: 550,  dist: 30 },
  'FULLERTON':          { base: 530,  dist: 26 },
  'GARDEN GROVE':       { base: 540,  dist: 28 },
  'GARDENA':            { base: 490,  dist: 10 },
  'GLENDALE':           { base: 540,  dist: 30 },
  'GLENDORA':           { base: 540,  dist: 34 },
  'HACIENDA HEIGHTS':   { base: 520,  dist: 26 },
  'HACIENDA HTS':       { base: 520,  dist: 26 },
  'HARBOR CITY':        { base: 450,  dist: 8  },
  'HAWTHORNE':          { base: 490,  dist: 14 },
  'HUNTINGTON BEACH':   { base: 500,  dist: 24 },
  'HUNTINGTON PARK':    { base: 490,  dist: 12 },
  'INGLEWOOD':          { base: 490,  dist: 12 },
  'IRVINE':             { base: 570,  dist: 35, requiresDropFee: true, dropFeeAmount: 150 },
  'IRWINDALE':          { base: 550,  dist: 26 },
  'JURUPA VALLEY':      { base: 750,  dist: 50 },
  'LA MIRADA':          { base: 500,  dist: 18 },
  'LA PUENTE':          { base: 540,  dist: 24 },
  ...
  'REDLAND':            { base: 740,  dist: 68, requiresDropFee: true, dropFeeAmount: 150 },
  'REDLANDS':           { base: 740,  dist: 68, requiresDropFee: true, dropFeeAmount: 150 },
  ...
  'SANTA FE SPRING':    { base: 490,  dist: 15 },
  'SIGNAL HILL':        { base: 470,  dist: 10 },
  'SOUTH EL MONTE':     { base: 510,  dist: 20 },
  'SOUTH GATE':         { base: 500,  dist: 12 },
  'STANTON':            { base: 520,  dist: 24 },
  'SUN LAND':           { base: 820,  dist: 30, requiresDropFee: true, dropFeeAmount: 150 },
  'SUNLAND':            { base: 820,  dist: 30, requiresDropFee: true, dropFeeAmount: 150 },
  'SUN VALLEY':         { base: 820,  dist: 32, requiresDropFee: true, dropFeeAmount: 150 },
  'TEMPLE CITY':        { base: 500,  dist: 22 },
  'TORRANCE':           { base: 510,  dist: 14 },
  'TUSTIN':             { base: 550,  dist: 30 },
  'UPLAND':             { base: 620,  dist: 40 },
  'VENTURA':            { base: 720,  dist: 65, requiresDropFee: true, dropFeeAmount: 150 },
  'VERNON':             { base: 550,  dist: 12 },
  'WALNUT':             { base: 560,  dist: 28 },
  'WEST COVINA':        { base: 530,  dist: 26 },
  'WESTMINSTER':        { base: 520,  dist: 26 },
  'WHITTIER':           { base: 500,  dist: 20 },
  'WILMINGTON':         { base: 460,  dist: 6  },
  'WILLINGTON':         { base: 460,  dist: 6  },
  'YORBA LINDA':        { base: 610,  dist: 32 },
}

// ── Weight surcharges (CHN Rate Sheet) ────────────────────────────────────────
export const DRAYAGE_WEIGHT_SURCHARGES = [
  { minLbs: 43001, maxLbs: 47000, surcharge: 250 },
  { minLbs: 47001, maxLbs: 50000, surcharge: 350 },
  { minLbs: 50001, maxLbs: Infinity, surcharge: 450 },
]

// ── Base add-ons (included in standard quotes) ────────────────────────────────
export const DRAYAGE_BASE_ADD_ONS = [
  { id: 'drayage.addons.pier-pass',    label: 'Prepaid Pier Pass',        amount: 80,  unit: 'USD/move' },
  { id: 'drayage.addons.tcf',          label: 'TCF (Terminal Clean Fuel)', amount: 20,  unit: 'USD/move' },
  { id: 'drayage.addons.chassis-split',label: 'Chassis Split',             amount: 100, unit: 'USD/move' },
  { id: 'drayage.addons.rush-request', label: 'Rush Fee',                  amount: 200, unit: 'USD/move' },
]

// ── Per-use invoice add-ons ───────────────────────────────────────────────────
export const DRAYAGE_INVOICE_ADD_ONS = [
  { id: 'drayage.invoice-addons.chassis-per-day',       label: 'Chassis Per Day (min. 2 days)',     amount: 45,  unit: 'USD/day'  },
  { id: 'drayage.invoice-addons.wccp-per-day',          label: 'WCCP Chassis Per Day (5–15 days)',  amount: 75,  unit: 'USD/day'  },
  { id: 'drayage.invoice-addons.waiting-per-hour',      label: 'Terminal Waiting Time (free 2 hrs)',amount: 65,  unit: 'USD/hour' },
  { id: 'drayage.invoice-addons.live-unload-per-hour',  label: 'Live Unload (free 1 hr)',           amount: 85,  unit: 'USD/hour' },
  { id: 'drayage.invoice-addons.extra-stop',            label: 'Extra Stop',                        amount: 150, unit: 'USD/stop' },
]

// ── Flat-fee items ────────────────────────────────────────────────────────────
export const DRAYAGE_FLAT_FEES = [
  { id: 'drayage.flat.storage-per-day',    label: 'Storage Fee',                 amount: 50,  unit: 'USD/day'  },
  { id: 'drayage.flat.empty-storage',      label: 'Empty Storage Fee',           amount: 50,  unit: 'USD/day'  },
  { id: 'drayage.flat.prepull',            label: 'Prepull',                     amount: 150, unit: 'USD/move' },
  { id: 'drayage.flat.dry-run',            label: 'Dry Run Terminal Charge',     amount: 150, unit: 'USD/move' },
  { id: 'drayage.flat.exam-fee',           label: 'Exam Fee',                    amount: 350, unit: 'USD'      },
  { id: 'drayage.flat.re-plug-in',         label: 'Re Plug In',                  amount: 85,  unit: 'USD/move' },
  { id: 'drayage.flat.do-cancellation',    label: 'DO Cancellation',             amount: 50,  unit: 'USD'      },
  { id: 'drayage.flat.on-time-delivery',   label: 'On Time Delivery Service',    amount: 150, unit: 'USD/move' },
]
...
export function calculateDrayageQuote(extraction: DrayageExtraction): DrayageQuoteResult {
  ...
  // Rush request
  if (rushRequest) {
    lineItems.push({ code: 'RUSH', description: 'Rush Fee', amount: 200 })
  }

  // Chassis split
  if (chassisSplitRequired) {
    lineItems.push({ code: 'CSPLIT', description: 'Chassis Split', amount: 100 })
  }

  // Pier pass
  if (prepaidPierPass !== false) {
    lineItems.push({ code: 'PP', description: 'Prepaid Pier Pass', amount: 80 })
  }

  // TCF
  if (tcfCharge !== false) {
    lineItems.push({ code: 'TCF', description: 'Terminal Clean Fuel (TCF)', amount: 20 })
  }

  // Chassis
  if (chassisDays && chassisDays > 0) {
    lineItems.push({ code: 'CHAS', description: `Chassis Days (${chassisDays} days)`, amount: chassisDays * 45 })
  }

  // WCCP chassis
  if (chassisDaysWccp && chassisDaysWccp > 0) {
    lineItems.push({ code: 'WCCP', description: `WCCP Chassis Days (${chassisDaysWccp} days)`, amount: chassisDaysWccp * 75 })
  }

  // Waiting time (free first 2 hours)
  if (waitingHours && waitingHours > 0) {
    const billableHours = Math.max(0, waitingHours - 2)
    if (billableHours > 0) {
      lineItems.push({ code: 'WAIT', description: `Terminal Waiting Time (${waitingHours} hrs, 2 free)`, amount: billableHours * 65 })
    }
  }

  // Live unload (free first 1 hour)
  if (liveUnloadHours && liveUnloadHours > 0) {
    const billableHours = Math.max(0, liveUnloadHours - 1)
    if (billableHours > 0) {
      lineItems.push({ code: 'LU', description: `Live Unload (${liveUnloadHours} hrs, 1 free)`, amount: billableHours * 85 })
    }
  }

  // Extra stops
  if (extraStops && extraStops > 0) {
    lineItems.push({ code: 'ESTOP', description: `Extra Stop(s) (${extraStops})`, amount: extraStops * 150 })
  }
